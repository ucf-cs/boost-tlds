#ifndef ONFINISHALLOCATOR_H
#define ONFINISHALLOCATOR_H
...
class OnFinishAllocator 
{
public:
    OnFinishAllocator(uint64_t totalBytes, uint64_t threadCount, uint64_t typeSize)
    {
...
#ifdef FINE
        printf("OnFinish [base: %p | pool: %p] : ", m_base, m_pool);
...
template<typename T>
uint64_t OnFinishAllocator<T>::allocs = 0;

template<typename T>
__thread char* OnFinishAllocator<T>::m_base;

template<typename T>
__thread uint64_t OnFinishAllocator<T>::m_freeIndex;
