...
{
public:
    NodeAllocator(uint64_t totalBytes, uint64_t threadCount, uint64_t typeSize)
    {
...
#ifdef FINE
        printf("Node     [base: %p | pool: %p] : ", m_base, m_pool);
...
template<typename T>
uint64_t NodeAllocator<T>::allocs = 0;

template<typename T>
__thread char* NodeAllocator<T>::m_base;

template<typename T>
__thread uint64_t NodeAllocator<T>::m_freeIndex;
