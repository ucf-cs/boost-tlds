#ifndef NODEDESCALLOCATOR_H
#define NODEDESCALLOCATOR_H
...
class NodeDescAllocator 
{
public:
    NodeDescAllocator(uint64_t totalBytes, uint64_t threadCount, uint64_t typeSize)
    {
...
#ifdef FINE
        printf("NodeDesc [base: %p | pool: %p] : ", m_base, m_pool);
...
template<typename T>
uint64_t NodeDescAllocator<T>::allocs = 0;

template<typename T>
__thread char* NodeDescAllocator<T>::m_base;

template<typename T>
__thread uint64_t NodeDescAllocator<T>::m_freeIndex;
