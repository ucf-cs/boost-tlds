/******************************************************************************
 * skip_cas.c // TODO: This looks like it was copied from the Fraser code
 *            // We need to update this with the license our project uses
 * 
...
ReturnCode TransSkip::Insert(setkey_t key, Desc* desc, uint8_t opid,
...
ReturnCode TransSkip::Delete(setkey_t key, Desc* desc, uint8_t opid, Node*& deleted, Node*& pred)
{
    deleted = NULL;
    bool ret = false;
    NodeDesc* nodeDesc = NULL;
...
ReturnCode TransSkip::Find(setkey_t key, Desc* desc, uint8_t opid, int &value)
{
    NodeDesc* nodeDesc = NULL;
...
Node* TransSkip::weak_search_predecessors(setkey_t k, Node* *pa, Node* *na)
{
    Node* x, *x_next;
    setkey_t  x_next_k;
    int        i;
...
Node* TransSkip::strong_search_predecessors(setkey_t k, Node* *pa, Node* *na)
{
...
void TransSkip::mark_deleted(Node* x, int level)
{
    Node* x_next;
...
inline bool TransSkip::IsSameOperation(NodeDesc* nodeDesc1, NodeDesc* nodeDesc2)
{
    return nodeDesc1->desc == nodeDesc2->desc && nodeDesc1->opid == nodeDesc2->opid;
}

inline bool TransSkip::DoesNodeExist(Node* node, setkey_t key)
{
    return node != NULL && node->key == key;
}

inline bool TransSkip::FinishPendingTxn(NodeDesc* nodeDesc, Desc* desc)
{
...
void TransSkip::free_node(ptst_t *ptst, Node* n)
{
    fr_gc_free(ptst, (void *)n, gc_id[(n->level & LEVEL_MASK) - 1]);
}

Node* TransSkip::alloc_node(ptst_t *ptst)
{
    int l;
    Node *n;
    l = get_level(ptst);
...
int TransSkip::get_level(ptst_t *ptst)
{
...
int TransSkip::check_for_full_delete(Node* x)
{
    int level = x->level;
...
void TransSkip::do_full_delete(ptst_t *ptst, Node* x, int level)
{
    int k = x->key;
#ifdef WEAK_MEM_ORDER
...
void TransSkip::init_transskip_subsystem(void)
{
    int i;
...
void TransSkip::destroy_transskip_subsystem(void)
{
    fr_destroy_gc_subsystem();
}

inline bool TransSkip::IsKeyExist(NodeDesc* nodeDesc)
{
...
inline bool TransSkip::IsNodeActive(NodeDesc* nodeDesc)
{
    return nodeDesc->desc->status == COMMITTED;
}

inline bool TransSkip::DoesKeyExist(NodeDesc* nodeDesc)
{
    bool isNodeActive = IsNodeActive(nodeDesc);
    uint8_t opType = nodeDesc->desc->ops[nodeDesc->opid].type;
...
inline void TransSkip::LocatePred(Node*& pred, Node*& curr, setkey_t key)
{
    Node* pred_next;
...
TransSkip* TransSkip::Allocate(TransactionHandler* transaction)
{
    TransSkip* transSkip;
    Node *n;
    int i;
    ...
    transSkip = (TransSkip*)malloc(sizeof(*transSkip) + (NUM_LEVELS-1)*sizeof(Node *));
    transSkip->m_head.key = SENTINEL_KEYMIN;
    transSkip->m_head.level = NUM_LEVELS;
    for ( i = 0; i < NUM_LEVELS; i++ )
    {
        transSkip->m_head.next[i] = n;
    }

    transSkip->m_tail = n;

    transSkip->m_nodeDescAllocator = new Allocator<NodeDesc>(transaction->cap * transaction->threadCount *  NodeDesc::SizeOf() * transaction->transSize, transaction->threadCount, NodeDesc::SizeOf());

    transSkip->transaction = transaction;

    new (transSkip) TransSkip;
